...
csar.max_units = 15 --number of pilots that can be carried
...
function csar.checkDisabledAircraftStatus(_args)
    ...
    if _unit ~= nil and _unit:getPlayerName() ~= nil and _playerName == _unit:getPlayerName() then
        ...
        if _lives == nil or _lives > 1 then
			...
			local _text = string.format("Welcome to DCS Indonesia - Garuda virtual Air WIng! Check briefing and guideline on this server. \n\nYou have " .. (_lives - 1) .. " lives remaining. Make sure you eject! \n\nWeapon Limitation ACTIVE, check radio menu F10 and valdiate your loadout!. \n\nMission Active can be found at radio menu F10, it will be auto-generate, just makes yourself assigned. \n\nIf You have any questions and ideas, feel free to contact us! \n\nGVAW - DSC Indonesia ")

            csar.displayMessageToSAR(_unit, _text, 30, true)
...
function csar.spawnGroup(_deadUnit)
    ...
    local _groupName = "DownedPilot #" .. _id
...
function csar.pickupUnit(_heliUnit, _pilotName, _woundedGroup, _woundedGroupName)
    ...
    csar.displayMessageToSAR(_heliUnit, string.format("%s: %s I'm in! Get to nearby the Airbase ASAP! ", _heliName, _pilotName), 10)
...
function csar.checkOnboard(_unitName)
    ...
    if _inTransit == nil or csar.tableLength(_inTransit) == 0 then
        ...
        local _text = "Onboard - RTB to FARP/Airfield or Command Center: "
