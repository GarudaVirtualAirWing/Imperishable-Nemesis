...
-- Usage of this script should credit the following contributors:
 ...
 -----------------------------------
 local version = "1.1 - March 2020" -- GVAW Initial Modification on 9NOV20
...
if file_exists("SaveUnits_GVAW1.lua") then --Script has been run before, so we need to load the save
...
--  AllGroups = SET_GROUP:New():FilterCategories("ground"):FilterActive(true):FilterStart()
  ...
  AllGroups = SET_GROUP:New():FilterPrefixes( {"BlueDefender ","RedDefender ", "CTLD_", "Resupply ", "AirbaseDef "} ):FilterActive(true):FilterStart() -- NAMA GROUP
  
    AllGroups:ForEachGroup(function (grp)
      grp:Destroy()
    end)

  dofile("SaveUnits_GVAW1.lua")
  tempTable={}
  Spawn={}
--RUN THROUGH THE KEYS IN THE TABLE (GROUPS)
  for k,v in pairs (SaveUnits_GVAW1) do
    units={}
--RUN THROUGH THE UNITS IN EACH GROUP
      for i= 1, #(SaveUnits_GVAW1[k]["units"]) do 
  
tempTable =

  { 
   ["type"]=SaveUnits_GVAW1[k]["units"][i]["type"],
   ["transportable"]= {["randomTransportable"] = false,}, 
   --["unitId"]=9000,used to generate ID's here but no longer doing that since DCS seems to handle it
   ["skill"]=SaveUnits_GVAW1[k]["units"][i]["skill"],
   ["y"]=SaveUnits_GVAW1[k]["units"][i]["y"] ,
   ["x"]=SaveUnits_GVAW1[k]["units"][i]["x"] ,
   ["name"]=SaveUnits_GVAW1[k]["units"][i]["name"],
   ["heading"]=SaveUnits_GVAW1[k]["units"][i]["heading"],
...
groupData = 

  {
    ...
    ["hidden"] = false,
    ["units"] = units,
    ["y"] = SaveUnits_GVAW1[k]["y"],
    ["x"] = SaveUnits_GVAW1[k]["x"],
    ["name"] = SaveUnits_GVAW1[k]["name"],
    --["start_time"] = 0,
  } 

  coalition.addGroup(SaveUnits_GVAW1[k]["CountryID"], SaveUnits_GVAW1[k]["CategoryID"], groupData)
  groupData = {}
...
--  AllGroups = SET_GROUP:New():FilterPrefixes( {"SAM", "MBT", "APC", "IFV"} ):FilterActive(true):FilterStart()
  AllGroups = SET_GROUP:New()
    :FilterPrefixes( {"RedDefender ", "BlueDefender ", "CTLD_", "Resupply "} ) -- NAMA GROUP
...
newMissionStr = IntegratedserializeWithCycles("SaveUnits_GVAW1",SaveUnits_GVAW1) --save the Table as a serialised type with key SaveUnits
writemission(newMissionStr, "SaveUnits_GVAW1.lua")--write the file from the above to SaveUnits.lua
SaveUnits_GVAW1={}--clear the table for a new write.
env.info("Data saved.")
